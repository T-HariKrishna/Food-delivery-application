<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%@ page import="com.hari.daoimplementaion.*" %>
<!DOCTYPE html>
<html>
  <head>
    <meta charset="UTF-8" />
...
h1 {
  ...
  <%RestaurantDaoImplementation restaurantDao=new RestaurantDaoImplementation();
    int restId=(int) request.getSession().getAttribute("restaurantId");%>
    <div class="form-container">
     <h1>Add Menu Details</h1>
           <form action="AddMenu" method="post">
      
        <label for="restaurantId">Restaurant ID:</label>
        <input type="text" id="restaurantId" name="restaurantId" value="<%=(int) request.getSession().getAttribute("restaurantId") %>"  readonly/>
      
       <label for="restaurantId">Restaurant Name:</label>
        <input type="text" id="restaurantName" name="restaurantName"  value="<%= restaurantDao.getRestaurantName(restId) %>" readonly/>
